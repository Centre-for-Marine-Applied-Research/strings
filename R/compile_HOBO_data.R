...
#'   default \code{rm.DST = FALSE} is used.
#'
#' @param correct.DO Logical value. If \code{TRUE}, dissolved oxygen will be
...
#'
#' @param export.csv Logical value indicating whether to export the compiled
...
compile_HOBO_data <- function(path.HOBO,
  ...
  for(i in seq_along(dat.files)) {
    ...
    # extract temperature/DO column header (includes units)
    unit_ref <- hobo.i_dat %>%
      colnames() %>%
      data.frame() %>%
      dplyr::rename(COL_NAME = 1) %>%
      filter(str_detect(COL_NAME, "Temp") |
               str_detect(COL_NAME, "DO")) %>%
      separate(col = COL_NAME, into = c("unit_ref", NA), sep = " \\(")
    ...
    if(isTRUE(correct.DO) & "Dissolved Oxygen" %in% names(hobo.i)) {
      ...
      if(is.null(Sal)){
        ...
        stop("Can't calculate salinity correction factor.
             \nHINT: Enter a value for the Sal argument or set correct.DO to FALSE")
      }
    ...
    for(j in seq_along(vars.to.select)){
      ...
      if(var.j == "Temperature") {
        var_ref <- unit_ref %>% filter(str_detect(unit_ref, "Temp"))

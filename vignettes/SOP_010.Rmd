...
```{css, echo=FALSE}
#re, code {white-space:pre !important; overflow-x:auto}
```
...
The purpose of this vignette is to provide detailed instruction on how to compile data from a deployment into a single spreadsheet, assuming the data is structured as described in [SOP009](file:///C:/Users/Danielle%20Dempsey/Desktop/RProjects/strings/vignettes/SOP_009.html). Section 3.1 provides a general overview of the `compile_***_data()` family of functions. Section 3.2 is CMAR-specific, and shows how to compile CMP data using a template.

See [SOP009](file:///C:/Users/Danielle%20Dempsey/Desktop/RProjects/strings/vignettes/SOP_009.html) for detailed description of the folder structure required to compile data using the `strings` package and SOP011 (vignette name) on how to calculate other parameters.
...
## 2. **SCOPE/PRINCIPAL** 
  ...
  1. CMAR Workflow: `Compile_Template.R`
...
This format is convenient for human readers, who can quickly scan the metadata to determine how many of this sensor type was deployed, the depths of deployment, etc. However, to include include the metadata, all values were converted to class character. To manipulate the data, the values should be converted to the appropriate class (e.g., POSIXct for the timestamps and
numeric for variable values), and the dataframe should be converted to a "long" format. This can be done using the `convert_to_tidydata()` function, which returns a dataframe with the following columns:

* DEPLOYMENT_RANGE: The deployment and retrieval dates.
* SENSOR: The sensor that recorded the measurement.
* TIMESTAMP: The timestamp of the measurement.
* VARIABLE: The parameter measured (Temperature, Dissolved Oxygen, or Salinity).
...
As with the other `compile` functions, `compile_all_data` returns the data in a wide format, with metadata in the first four rows. The columns alternate between timestamp (in the format "Y-m-d H:M:S") and variable value (rounded to three decimal places). Note that because the sensors can be initialized at different times and because the sensors record on different time intervals, values in a single row do not necessarily correspond to the same timestamp.  
...
Before compiling:
    1. Make sure your data and folder structure is as described in Section 3.1 of [SOP009](file:///C:/Users/Danielle%20Dempsey/Desktop/RProjects/strings/vignettes/SOP_009.html).
    1. Fill out the STRING TRACKING spreadsheet to make sure all of the data is in the appropriate folders. You may need to add a new row for the deployment you are compiling.

Compiling data for a single deployment is straightforward using the `Compile_Template.R` saved in the Data_Strings folder. You should only need to change one line, but we will walk through each of the sections here.

Open `Compile_Template.R`.

Save the file into the deployment folder with the name structure: “Compile_Deployment_Location_YYYY-MM-DD.” 
...
**Section 2:**
Run this section to extract information about the deployment from the deployment log. This information will be used as arguments in the `compile_all_data()` function.
...
The function will automatically trim data to deployment and retrieval dates included in the log (supplied in the `deployment.,range` argument), in case sensors were turned on prior to deployment date or left running after retrieval.   
...
plot_variables_at_depth(ALL_raw_tidy,
                        ...
                        ylab.units = c("degC", "(%)", "(ppt)"))

```

You will get a note from ggplot if any dissolved oxygen values are >130 % (the upper limit on the y-axis of the plot.) These values will be removed when the data is trimmed (see SOP011).

...
The purpose of this vignette is to provide detailed instruction on how to trim compiled CMP data following CMAR's workflow. The trimmed data is exported to the deployment folder and also formatted and exported to the appropriate county folder for transfer to the Open Data Portal. 

Detailed instruction on how to compile data is in [SOP010](file:///C:/Users/Danielle%20Dempsey/Desktop/RProjects/strings/vignettes/SOP_010.html). 

Section 3.1 provides a general overview of the `trim_data()` function. Section 3.2 is CMAR-specific, and shows how to trim and export CMP data using a template.
 
## 2. **SCOPE/PRINCIPAL** 

  1. `trim_data()` function
  1. CMAR Workflow: `Trim_Template_OpenData.R`
...
```{r}
# in this example, all variables are trimmed to the same start and end times. 
# This does not have to be the case - you can provide different start/end times for each variable 
# in the different sections of the pipe.

# specify the timestamp for the FIRST VALID measurement
# (this can be copy/pasted from the _raw.csv file)
start.timestamp <- "2019-07-12  7:48:51 PM"

# specify the timestamp for the LAST VALID measurement 
# (this can be copy/pasted from the _raw.csv file)
last.timestamp <- "2019-10-31  2:30:22 PM"
...
### 3.2 CMAR Workflow: `Trim_Template_OpenData.R`

Before trimming:
* Check the STRING TRACKER to determine which deployments are a priority for sending to Open Data.

Trimming data for a single deployment should be done by following `Trim_Template_OpenData.R` saved in the Data_Strings folder. This will export the trimmed data to the deployment folder and the Open Data folder. We will walk through each of the sections here.

Open `Trim_Template_OpenData.R` from the Data_Strings folder.

Save the file into the deployment folder with the name structure: “Trim_Deployment_Location_YYYY-MM-DD.” 

![Figure 1. Save `Trim_Template_OpenData.R` into the deployment folder of the string you are processing and re-name.](SOP011_figure1.png)

At the top of `Trim_Template_OpenData.R`, fill in the current date, your name, the version of the `strings` package you are using, and any additional notes you deem necessary for future data management. 
...
**Section 1:**
Update the `path` variable to the deployment folder you are processing. You do no need to change the `path.export` variable.
...
path <- file.path("Y:/Coastal Monitoring Program/Data_Strings/Madeline Point/Madeline Point 2019-04-25")
...
```{r, echo=FALSE}
dat_raw <- wide_data[, c(1:4, 7:20)] 
```

Convert to long (tidy) format and plot to get a feel for where to trim the data. Modify `vars.to.plot` for the variables included in the data. In our example, sensors measured temperature, dissolved oxygen, and salinity.
...
sensors.DO <- c("aquaMeasure")
start.DO <- "2019-05-02  8:45:22 PM"
...
The dissovled oxygen signal is consistent with biofouling and should be trimmed farther. See Open Data/Guysborough/Guysborough_Trim_Meeting.pptx for common dissolved oxygen signals and where to trim.
...
```{r, eval=FALSE}
# export trimmed data
write_csv(dat_trim, path = paste(path, "/", file.name, "_trimmed.csv", sep = ""))
...
write_csv(dat_open, path = paste(path.export, "/", open.data.name, ".csv", sep = ""))

```

Make sure you indicate on the STRINGS TRACKING sheet that this deployment has been trimmed.
